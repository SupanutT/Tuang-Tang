...
module.exports.showBill = async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ message: `Found one with id : ${req.params.id}`, data });
};
...
module.exports.createBill = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(400)
      .send({ message: "Image is NOT uploaded. Please try again!" });
  }
  const urlParts = bill.image.url.split("/");
  const filenameWithType = urlParts[urlParts.length - 1];
  const { cleanedData, merchantName } = await getItemDetail(
    filenameWithType,
    bill.image.url
  );
  bill.all_dividers = [];
  ...
  await bill.save();
  res.status(200).send({
    message: `Bill ${bill._id} is successfully created`,
...
module.exports.updateBill = async (req, res, next) => {
  ...
  await bill.save();
  res.status(200).send({
    message: `Bill ${id} is successfully updated`,
    data: { bill_id: bill._id },
  });
};
