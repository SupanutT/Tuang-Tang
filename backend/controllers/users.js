...
module.exports.register = async (req, res) => {
  ...
  try {
    ...
    // console.log("10", username, email, password, name);
    const user = new User({ email, username, name });
    // console.log("12", user);
    const registeredUser = await User.register(user, password);
    // console.log("14", registeredUser);
...
module.exports.login = async (req, res) => {
  ...
  const accessToken = jwt.sign({ id: user._id.toString() }, "your-secret-key", {
    expiresIn: "15m",
  });
  const refreshToken = jwt.sign({ id: user._id.toString() }, "refresh-secret", {
    expiresIn: "7d",
  });
...
module.exports.refreshToken = async (req, res) => {
  ...
  if (refreshToken && refreshToken.status === "issued") {
    ...
    const refreshTokens = await RefreshToken.updateMany(
      { id: userId },
    ...
    res.status(403).json({
      message:
        "Session is already used by another user, please change the password immediately.",
    });
  } else {
